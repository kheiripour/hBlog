...
class PostAdmin(admin.ModelAdmin):
    fields = ("active_version", "author", "status", "pub_date")
    list_display = (
        "active_version",
        "id",
        "author",
    ...
    def render_change_form(self, request, context, *args, **kwargs):
        context["adminform"].form.fields[
            "active_version"
        ].queryset = PostVersion.objects.filter(post=context["object_id"])
...
class CommentAdmin(admin.ModelAdmin):
    ...
    def render_change_form(self, request, context, *args, **kwargs):
        ...
        context["adminform"].form.fields[
            "replied_to"
        ].queryset = Comment.objects.filter(post=obj.post, replied_to=None).exclude(
            id=obj.id
        )
        return super(CommentAdmin, self).render_change_form(
            request, context, *args, **kwargs
        )
