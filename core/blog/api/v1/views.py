...
class BlogModelViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def send_comment(self, request, pk=None):
        ...
        if replied_to and replied_to not in Comment.objects.filter(post=post):
            return Response(
                {"details": "replied_to not valid, must be in post comments"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {"details": "comment sent successfully."}, status=status.HTTP_201_CREATED
        )
        ...
        if request.method == "GET":
            return Response(
                PostVersionSerializer(
                    post_versions, many=True, context={"request": request}
                ).data,
            ...
            return Response(
                {"detail": "version request sent successfully"},
