...
class Command(BaseCommand):
    """
    Make and import fake post and comments by given number items.
    """
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(number):
            ...
            for _ in range(content_paras):
                content = (
                    content
                    + "<p>"
                    + fake.paragraph(nb_sentences=choice(range(10, 20)))
                    + "</p>"
                )
        ...
        self.stdout.write(
            self.style.SUCCESS("Successfully added %i posts and %i comments :)")
            % (number, all_comments)
        )
